...
<div class="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md border border-gray-100">
    ...
    <form action="{{ route('admin.youth.members.store') }}" method="POST">
        @csrf
        <!-- Hidden Input Kategori (Otomatis terisi sesuai URL) -->
        <input type="hidden" name="category" value="{{ $category }}">

        <div class="mb-4">
            <label class="block text-sm font-bold text-gray-700 mb-1">Nama Lengkap</label>
            <input type="text" name="name" class="w-full border rounded p-2.5" required>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div>
                ...
                <input type="date" name="birth_date" class="w-full border rounded p-2.5">
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">No. HP (WA)</label>
                <input type="text" name="phone" class="w-full border rounded p-2.5" placeholder="08xxxxx">
            </div>
        </div>

        <div class="flex justify-end gap-3">
            <a href="{{ url()->previous() }}" class="px-4 py-2 text-gray-600 font-bold hover:bg-gray-100 rounded">Batal</a>
