<tr class="hover:bg-gray-50 transition">
    ...
    <td class="px-4 py-4 border-b border-gray-200 text-sm text-center align-middle">
        <div class="flex items-center justify-center gap-2">
            ...
            <!-- Tombol Edit Baru -->
            <a href="{{ route('admin.liturgy.personnels.edit', $person->id) }}" class="text-blue-500 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg transition" title="Edit Data">
            ...
            <form action="{{ route('admin.liturgy.personnels.destroy', $person->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data {{ $person->name }}?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-2 rounded-lg transition flex items-center justify-center" title="Hapus">
