...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md border border-gray-100">
    ...
    <form id="activityForm" action="{{ route('admin.activities.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div>
                ...
                @if(in_array(Auth::user()->role, ['admin', 'pengurus_gereja', 'ketua_wilayah']))
                    <input type="text" name="organizer" 
                           value="{{ Auth::user()->role === 'ketua_wilayah' ? 'Wilayah ' . Auth::user()->territory->name : '' }}"
                           class="w-full border border-gray-300 rounded p-2.5 focus:ring-blue-500" 
                           placeholder="Contoh: OMK, Wilayah St. Yusuf" required>
                @else
                    <input type="text" value="{{ Auth::user()->role === 'ketua_lingkungan' ? 'Lingkungan ' . Auth::user()->lingkungan->name : $myOrg }}" class="w-full border rounded p-2.5 bg-gray-100 font-bold text-gray-600" disabled>
                @endif
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div><label class="block text-gray-700 text-sm font-bold mb-2">Waktu Mulai</label><input type="datetime-local" name="start_time" class="w-full border rounded p-2.5" required></div>
            <div><label class="block text-gray-700 text-sm font-bold mb-2">Waktu Selesai (Opsional)</label><input type="datetime-local" name="end_time" class="w-full border rounded p-2.5"></div>
        </div>

        <!-- Dropdown Lingkungan yang sudah difilter -->
        @if(in_array(Auth::user()->role, ['admin', 'pengurus_gereja', 'ketua_wilayah', 'ketua_lingkungan']))
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">Target Lingkungan (Opsional)</label>
            ...
            <p class="text-xs text-gray-500 mt-1">Pilih jika kegiatan ini spesifik untuk satu lingkungan.</p>
        </div>
        @endif

        <div class="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-200">
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" name="show_on_lingkungan_page" value="1" class="form-checkbox h-5 w-5 text-blue-600 rounded" checked>
                <span class="ml-3 text-gray-700 font-medium">Tampilkan di Halaman Detail Lingkungan?</span>
            </label>
        </div>

        <div class="mb-6">
            <label class="block text-gray-700 text-sm font-bold mb-2">Deskripsi & Foto Kegiatan</label>
            <textarea id="summernote" name="description" required></textarea>
        </div>

        <div class="mb-6">
            <label class="block text-gray-700 text-sm font-bold mb-2">Gambar Sampul (Thumbnail)</label>
            <input id="imageInput" type="file" name="image" class="w-full border rounded p-2 bg-gray-50 text-sm">
